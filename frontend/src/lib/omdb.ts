...
export async function omdbSearch(query: string): Promise<OMDBMovie[]> {
  if (!query) return []
  const data = await apiClient.post<{ Search: OMDBMovie[] }>(`/api/films/search`, { query }, { auth: false })
  return data?.Search || []
}

export async function omdbGetById(id: string): Promise<OMDBMovieDetail> {
  const data = await apiClient.post<OMDBMovieDetail>(`/api/films/detail`, { id }, { auth: false })
  return data
}
