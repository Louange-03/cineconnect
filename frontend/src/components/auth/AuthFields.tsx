...
export function Field({
    ...
    return (
        <div className="space-y-1">
            <label className="ml-0.5 text-xs font-medium uppercase tracking-wider text-gray-400">{label}</label>
            {children}
        </div>
    )
}
    ...
    return (
        <div className="relative">
            ...
            <input
                {...props}
                className={[
                    "w-full rounded-lg border border-white/10 bg-white/5 py-2 text-sm text-white placeholder:text-gray-500",
                    leftIcon ? "pl-9 pr-3" : "px-3",
                    "outline-none transition focus:border-[#3EA6FF]/40 focus:ring-1 focus:ring-[#1D6CE0]/35",
                    props.className ?? "",
                ].join(" ")}
            />
...
export function Alert({
    ...
    return (
        <div className={`flex items-start gap-2 rounded-md border px-2.5 py-1.5 text-xs ${styles}`}>
...
export function EyeButton({
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className="rounded-md px-1.5 py-1 text-white/55 transition hover:text-white focus:outline-none focus-visible:ring-1 focus-visible:ring-white/25"
