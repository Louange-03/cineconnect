...
const PAGES = Math.max(1, Number(process.env.SEED_TMDB_PAGES ?? "20"))
...
async function fetchPopularMovies(page: number): Promise<TmdbMovie[]> {
  const url = `${TMDB_BASE}/movie/popular?api_key=${TMDB_API_KEY}&language=${encodeURIComponent(LANGUAGE)}&page=${page}`
  const res = await fetch(url)
  if (!res.ok) {
    const text = await res.text().catch(() => "")
    throw new Error(`TMDB popular page=${page} error ${res.status}: ${text}`)
  }
...
async function run(): Promise<void> {
  ...
  console.log(`[seed:tmdb] start pages=${PAGES}, lang=${LANGUAGE}`)
  let imported = 0

  for (let page = 1; page <= PAGES; page++) {
    const list = await fetchPopularMovies(page)
    if (!list.length) {
      console.log(`[seed:tmdb] page ${page}: 0 résultat`)
      break
    }
    ...
    console.log(`[seed:tmdb] page ${page}/${PAGES} OK`)
  }

  console.log(`[seed:tmdb] terminé, nouveaux films importés: ${imported}`)
  process.exit(0)
}
